...
dungeon.setDescription("You awaken locked up in a cramped cell and can barely move. Your muscles ache and you feel uncomfortable as you see the rotting bones littering the corner. You think to yourself that there must be a way to escape.");
dungeon.setImage("./images/dungeon.png");

const kitchen = new Room("kitchen");
kitchen.setDescription("The large, cold room is crowded with cooking equipment but is completely deserted. A large pot of stew boils on a fire in the corner, unattended. The hunger consuming you, you take a few sips gratefully.");
kitchen.setImage("./images/kitchen.png");

const dininghall = new Room("dining hall");
dininghall.setDescription("This is where the lords of the castle dine. A long, imposing table is in the centre of the room. Gilded candlesticks and silver plates decorate the table. Memories come back to you of dining here as a youngster, before the troubles with the Queen.");
dininghall.setImage("./images/dininghall.png");

const ballroom = new Room("ballroom");
ballroom.setDescription("You enter the vast room and see chairs lining the perimeter. An ornate chandelier hangs from the ceiling. On special occasions, this room would be cleaned and decorated to perfection. Today, however, it is dank and dusty.");
ballroom.setImage("./images/ballroom.png");

const courtyard = new Room("courtyard");
courtyard.setDescription("The beautiful castle courtyard was your and your sister's personal playground when you were young. Brightly coloured flowers grow wherever you look and a large oak tree stands solitary in the centre.");
courtyard.setImage("./images/courtyard.png");

const throneroom = new Room("throne room");
throneroom.setDescription("You stand at the end of a long, imposing chamber: the throne room of the Queen herself. From the galleries above, the lords of the kingdom watch to see what actions you will take. You get the feeling there is only one way out...");
...
guard.setDescription("The slightly odd jailer that watches over the prisons. Currently, he is asleep on guard duty.");
...
peasant.setDescription("A common peasant who works in the caslte as a maid. She is going about her duties.");
peasant.setImage("./images/peasant.png");
peasant.setDialogue("Tread carefully! A powerful mage is guarding in the ballroom blocking entrance to the throne room.");
dininghall.setCharacter(peasant);

const mage = new Enemy("Daiv");
mage.setDescription("A powerful mage who gets in your way to prevent you leaving. He tries to curse you.");
...
queen.setDescription("The tyrant who rules the kingdom. Your loathed sister, who imprisoned you and took the throne that was rightfully yours.");
...
dragon.setDescription("The dreaded pet dragon, controlled only by the monarch of the kingdom. The one who tames Brion could control the entire kingdom.");
dragon.setImage("./images/dragon.png");
dragon.setDialogue("....");
courtyard.setCharacter(dragon);

const key = new Item("key");
key.setDescription("This key might just open your cell and allow you to escape.");
key.setImage("./images/key.png");
dungeon.setItem(key);

const staff = new Weapon("staff");
staff.setDescription("A simple, yet extremely deadly, mage's staff that allows you to cast basic spells.");
...
crown.setDescription("The enchanted crown worn by the ruler of the kingdom. Legends say it has magical properties.");
...
function submitAction() {
    ...
    if (directions.includes(action) && room.linkedRooms.get(action)) {    
        if (room === dungeon && !(inventory.innerText.includes("key"))) {
            alertMessage("You can't leave your cell without a key to unlock it.", "./images/lock.png");
        }    
        else if (safeToLeave) {
        ...
        else alertMessage("There is no one here to talk to.", "./images/invalid.png")
    }
...
function setRoom(room) {
    ...
    if (character) {     
        ...
        if ((character.type === "enemy" || character.type === "boss") && character.awake) {
            safeToLeave = false;
        }
    }
...
function giveItem(character, gift) {
    if (gift === "") return;
        ...
        inputModal.style.display = "none";
        giveBox.style.display = "none";
        alertMessage(`${character.name} is gratfeul for the gift and whispers in your ear: "${character.secret}"`, character.image);
    } 
...
function doBattle(character, spell) {
    if (character.type === "boss") {
        ...
        gameOver(`${character.name} will only obey the wearer of the Enchanted Crown of the Kingdom. ${character.name} becomes enraged and attacks you. Your journey ends in defeat.`);
    }
    else if (!(spells.innerText.includes(spell))) {
        inputModal.style.display = "none";
        castBox.style.display = "none";
        gameOver(`That is not a spell you know. ${character.name} takes advantage of your moment of hesitation and attacks you. Your journey ends in defeat.`);
    }
        ...
        inputModal.style.display = "none";
        castBox.style.display = "none";
        gameOver(`${character.name} resisted your ${spell} spell. ${character.name} attacks you in retaliation and kills you. Your journey ends in defeat.`);
    }
...
// - add sounds or music or backgrounds
// - mobile version
>>>>>>> f95faf0fa254e0fc3caee16ecb1f7803b60ab46c
